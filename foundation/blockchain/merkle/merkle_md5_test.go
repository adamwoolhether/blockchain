...
func TestMerkleTreeMD5Content_VerifyContent(t *testing.T) {
	for i := 0; i < len(tableMD5Content); i++ {
		...
		if len(tableMD5Content[i].contents) > 0 {
			v, err := tree.VerifyData(tableMD5Content[i].contents[0])
			if err != nil {
				t.Fatal(err)
			}
			if !v {
		...
		if len(tableMD5Content[i].contents) > 1 {
			v, err := tree.VerifyData(tableMD5Content[i].contents[1])
			if err != nil {
				t.Fatal(err)
			}
			if !v {
		...
		if len(tableMD5Content[i].contents) > 2 {
			v, err := tree.VerifyData(tableMD5Content[i].contents[2])
			if err != nil {
				t.Fatal(err)
			}
			if !v {
		...
		v, err := tree.VerifyData(tableMD5Content[i].notInContents)
		if err != nil {
			t.Fatal(err)
		}
		if v {
