...
func TestMerkleTreeSHA256Content_VerifyContent(t *testing.T) {
	for i := 0; i < len(tableSHA256Content); i++ {
		...
		if len(tableSHA256Content[i].contents) > 0 {
			v, err := tree.VerifyData(tableSHA256Content[i].contents[0])
			if err != nil {
				t.Fatal(err)
			}
			if !v {
		...
		if len(tableSHA256Content[i].contents) > 1 {
			v, err := tree.VerifyData(tableSHA256Content[i].contents[1])
			if err != nil {
				t.Fatal(err)
			}
			if !v {
		...
		if len(tableSHA256Content[i].contents) > 2 {
			v, err := tree.VerifyData(tableSHA256Content[i].contents[2])
			if err != nil {
				t.Fatal(err)
			}
			if !v {
		...
		v, err := tree.VerifyData(tableSHA256Content[i].notInContents)
		if err != nil {
			t.Fatal(err)
		}
		if v {
