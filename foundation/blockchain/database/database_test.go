	...
	for _, tst := range tt {
		f := func(t *testing.T) {
			...
			for _, tx := range tst.txs {
				...
				if err := db.ApplyTx(database.Block{Header: database.BlockHeader{BeneficiaryID: tst.miner}}, blockTx); err != nil {
...
func TestNonceValidation(t *testing.T) {
	...
	for _, tst := range tt {
		...
		for i, tx := range tst.txs {
			...
			err = db.ApplyTx(database.Block{Header: database.BlockHeader{BeneficiaryID: tst.miner}}, blockTx)
